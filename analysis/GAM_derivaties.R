...
plotGAMSignificantSlopes = function(gam.model, Term, Term.label, eps = 1e-7, aoaLab="AoAscale",subPlot=1,termPlot = NA,subPlotB=subPlot,interactionTermValue=NA,interactionTermValueByVar = list()){
  ...
  if(is.na(termPlot)){
    termPlot = which(t.labs==Term)
  }
  smooth.plot = plot(gam.model,select=termPlot)#,xlab=Term.label)
    ...
    if(!var %in% names.smooth.plot){
      ...
      if(!is.na(varNum)){
        newD[,var] = smooth.plot[[
          ...
          which(grepl(var,smooth.plot.VarNames))[subPlotB]
        ]]$raw[1]} else{
          # interaction term
          newD[,var] = interactionTermValue
          if(var %in% names(interactionTermValueByVar)){
            newD[,var] = interactionTermValueByVar[[var]]
          }
        }
    } else{
